...
export async function createPurchase(buyerId: number, data: CreatePurchaseRequest) {
  ...
  try {
    const result = await db.begin(async (tx: any) => {
      ...
      const buyers = await tx`SELECT id, email FROM users WHERE id = ${buyerId}`
      if (!buyers || buyers.length === 0) throw new Error('Buyer account not found.')
      ...
      const newTransaction = txnResult[0]
